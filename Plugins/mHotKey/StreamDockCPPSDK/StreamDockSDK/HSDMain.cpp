...
int esd_main(int argc, const char** argv, HSDBasePlugin* plugin) {
  if (argc != 9) {
    HSDLog("Invalid number of parameters {} instead of 9", argc);
    return 1;
  }
  ...
  if (port == 0) {
    HSDLog("Invalid port number");
    return 1;
  }

  if (pluginUUID.empty()) {
    HSDLog("Invalid plugin UUID");
    return 1;
  }

  if (registerEvent.empty()) {
    HSDLog("Invalid registerEvent");
    return 1;
  }
  ...
  HSDLocalizer::Initialize(language);
