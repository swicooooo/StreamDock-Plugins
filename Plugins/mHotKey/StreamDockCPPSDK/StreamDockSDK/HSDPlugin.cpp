...
void HSDPlugin::KeyDownForAction(
  ...
  if (!action) {
    HSDLog("No action for keydown - {} {}", inAction, inContext);
    return;
  }
...
void HSDPlugin::KeyUpForAction(
  ...
  auto action = GetOrCreateAction(inAction, inContext);
  if (!action) {
    HSDLog("No action for keyup - {} {}", inAction, inContext);
    return;
  }
...
void HSDPlugin::DialPressForAction(
  ...
  auto action = GetOrCreateAction(inAction, inContext);
  if (!action) {
    HSDLog("No action for dialPress - {} {}", inAction, inContext);
    return;
  }
...
void HSDPlugin::DialRotateForAction(
  ...
  auto action = GetOrCreateAction(inAction, inContext);
  if (!action) {
    HSDLog("No action for dialRotate - {} {}", inAction, inContext);
    return;
  }
...
void HSDPlugin::WillAppearForAction(
  ...
  auto action = GetOrCreateAction(inAction, inContext);
  if (!action) {
    HSDLog("No action for WillAppear - {} {}", inAction, inContext);
    return;
  }
...
void HSDPlugin::SendToPlugin(
  ...
  auto action = GetOrCreateAction(inAction, inContext);
  if (!action) {
    HSDLog("Received plugin request for unknown action {} {}", inAction, inContext);
    return;
  }
...
void HSDPlugin::DidReceiveSettings(
  ...
  auto action = GetOrCreateAction(inAction, inContext);
  if (!action) {
    HSDLog("No action for DidReceiveSettings: {} {}", inAction, inContext);
    return;
  }
