...
bool IsNetworkDriveRoot(const std::string& inUtf8Path) {
  ...
  if (pos == std::string::npos || pos == pathWithoutNetworkPrefix.length() - 1) {
    return true;
  }
...
std::string GetExtension(const std::string& inPath) {
  const std::string fileName = HSDUtilities::GetFileName(inPath);
...
void HSDUtilities::DoSleep(int inMilliseconds) {
  Sleep(inMilliseconds);
}

std::string HSDUtilities::GetFileName(const std::string& inPath) {
...
std::string HSDUtilities::AddPathComponent(
...
std::string HSDUtilities::GetParentDirectoryPath(const std::string& inPath) {
...
ESD::filesystem::path HSDUtilities::GetPluginExecutablePath() {
