...
void HSDConnectionManager::OnFail(WebsocketClient* inClient, websocketpp::connection_hdl inConnectionHandler) {
    ...
    HSDDebug("Failed with reason: {}", reason);

    if (mAsioContext) {
        HSDDebug("Stopping ASIO context");
        mAsioContext->stop();
    }
}
...
void HSDConnectionManager::OnClose(WebsocketClient* inClient, websocketpp::connection_hdl inConnectionHandler) {
    ...
    //if (reason == "app close")  //¾ÙÀý
    //{
    //    dynamic_cast<MyStreamDockPlugin*>(mPlugin)->Is_Exit = true;
    //}


	//dynamic_cast<MyStreamDockPlugin*>(mPlugin)->Is_Exit = true;

    HSDDebug("Close with reason: {}", reason);
...
void HSDConnectionManager::Run() {
    try {
        ...
        if (ec) {
            HSDDebug("Connect initialization error: {}", ec.message());
            return;
        }
        ...
        (void)e;
        HSDDebug("Websocket threw an exception: {}", e.what());
    }
}
