...
export default function (name: string) {
  ...
  useWatchEvent('action', {
    ...
    async willAppear({ context }) {
      ...
      try {
        ...
        if (!state) {
          ...
          volumeDbMap[context] = Number(result.inputVolumeDb).toFixed(1);
        } else {
          // settings.inputVolumeDb = 0;
          volumeDbMap[context] = 0;
        }
      ...
      plugin.Interval(context, 1000, async () => {
        ...
        try {
          ...
          if (!state) {
            volumeDbMap[context] = Number(result.inputVolumeDb).toFixed(1);
            plugin.getAction(context).setTitle(volumeDbMap[context] + ' db');
            // settings.inputVolumeDb = result.inputVolumeDb;
            // plugin.getAction(context).setSettings(settings);
          } else {
    ...
    async dialDown({ payload, context }) {
    ...
    async dialRotate({ payload, context }) {
      ...
      let inputVolumeDb = Number.parseInt(volumeDbMap[context]);

      let step = Number.parseInt(settings.step);
      let sumInputVolumeDb = inputVolumeDb + (ticks * step);
      ...
      volumeDbMap[context] = sumInputVolumeDb;
