...
Action *ExamplePlugin::GetOrCreateAction(const QString &action, const QString &context)
{
    ...
    if (action == "com.hotspot.stream.wavelinkInput" ||
        action == "com.hotspot.stream.wavelinkOutput" ||
        action == "com.hotspot.stream.wavelinkDeivce" ||
        action == "com.hotspot.stream.wavelinkEffects") {
