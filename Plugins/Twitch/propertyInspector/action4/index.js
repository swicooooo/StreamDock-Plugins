...
const $propEvent = {
    ...
    didReceiveSettings({ settings }) {
        if ('followerDuration' in settings) {
            $dom.followerDuration.value = settings.followerDuration;
        }
    },
...
// 时间限制
$dom.followerDuration.on('change', (event) => {
    $settings.followerDuration = event.target.value;
})
...
var index = 0;
function openRemindinWindow4(messageObj) {
