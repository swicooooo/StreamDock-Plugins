...
const $propEvent = {
    ...
    didReceiveSettings({ settings }) {
        if ('commercialTime' in settings) {
            $dom.commercialTime.value = settings.commercialTime;
        }
    },
...
// 时间限制
$dom.commercialTime.on('change', (event) => {
    $settings.commercialTime = event.target.value;
})
...
var index = 0;
function openRemindinWindow5(messageObj) {
