...
const $propEvent = {
    ...
    didReceiveSettings({ settings }) {
        if ('waitTime' in settings) {
            if (settings.waitTime === 0) {
                settings.waitTime = 3;
                $dom.waitTime.value = 3;

            } else {
                $dom.waitTime.value = settings.waitTime;
...
// 时间限制
$dom.waitTime.on('change', (event) => {
    $settings.waitTime = event.target.value;
})
...
var index = 0;
function openRemindinWindow6(messageObj) {
