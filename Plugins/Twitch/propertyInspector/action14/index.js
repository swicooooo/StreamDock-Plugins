...
const $propEvent = {
  ...
  didReceiveSettings({ settings }) {
    $dom.describe.value = settings.describe;
  },
...
$dom.user.on("click", function () {
  ...
  $websocket.openUrl(
    "http://localhost:3000/validate?code=" + tempToken,
    Date.now()
  );
});
...
$dom.describe.on("input", function () {
  $settings.describe = $dom.describe.value;
});
// 传递方法名和参数
...
var index = 0;
function openRemindinWindow14(messageObj) {
