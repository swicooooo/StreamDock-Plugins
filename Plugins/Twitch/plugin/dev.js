...
try {
  ...
  const content = `const { spawnSync } = require('node:child_process');
const target = '${path.join(__dirname, "index.js").replaceAll("\\", "\\\\")}';
const args = [
  target,
  ...process.argv.slice(2),
];
const ret = spawnSync("node.exe", args, { stdio: 'inherit', windowsHide: true,cwd:"${process
    .cwd()
    .replaceAll("\\", "\\\\")}" });
