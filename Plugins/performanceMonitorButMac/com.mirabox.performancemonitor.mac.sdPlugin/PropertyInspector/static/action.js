...
const onFilePickerReturn = (url) => {
    ...
    if (typeof url === 'string') {
        try {
            ...
            console.error("Invalid JSON string in onFilePickerReturn:", url, e);
            return;
        }
    } else if (Array.isArray(url)) {
        parsedUrl = url;
    } else {
        console.error("Invalid url type in onFilePickerReturn:", typeof url, url);
        return;
    }
    console.log(parsedUrl);
    $emit.send(`File-${$FileID}`, parsedUrl);
}