...
void HSDConnectionManager::AddDevice(const device& dev) {
    std::lock_guard<std::mutex> lock(deviceMutex);
    for (const auto& d : deviceList) {
        if (d.context == dev.context && d.deviceID == dev.deviceID) {
            return;
        }
    }

    deviceList.push_back(dev);
    HSDLogger::LogMessage("Device added: " + dev.context + " (" + dev.deviceID + ")");
}

void HSDConnectionManager::RemoveDeviceByContext(const std::string& context) {
    std::lock_guard<std::mutex> lock(deviceMutex);
    deviceList.erase(
        std::remove_if(deviceList.begin(), deviceList.end(),
            [&](const device& d) { return d.context == context; }),
        deviceList.end()
    );
    HSDLogger::LogMessage("Device removed: " + context);
}
...
void HSDConnectionManager::OnMessage(websocketpp::connection_hdl, WebsocketClient::message_ptr inMsg) {
    if (inMsg && inMsg->get_opcode() == websocketpp::frame::opcode::text) {
        ...
        try {
            ...
            {
                if (event == "willAppear" && action == "com.hotspot.stream.chroma") {
                    AddDevice({ context, action, deviceID });
                }
                else if (event == "willDisappear") {
                    RemoveDeviceByContext(context);
                }
            }
...
void HSDConnectionManager::sendToDevice(
    const std::string& context,
    const std::string& device,
    const std::string& deviceID,
    const std::vector<int>& lampValues, // r,g,b,r,g,b...
    bool dependenceDevice)
{
    json payload;
    payload["deviceID"] = deviceID;
    if (dependenceDevice) {
        payload["Dependence"] = "device";
    }
    payload["controlType"] = "lampControl";
    payload["lampType"] = "N4ProLampControl";
    payload["lampControlArray"] = lampValues;

    json jsonObject;
    jsonObject["event"] = "sendToDevice";
    jsonObject["context"] = context;
    jsonObject["device"] = device;
    jsonObject["payload"] = payload;

    //HSDLogger::LogMessage("发送的数据是：：：：：" + jsonObject.dump());
